    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            if self.write_interval > 0:
                ...
                self.track_data("Policy / Entropy Scaled (max)",
                                torch.max(policy_entropy_scaled).item())
                self.track_data("Policy / Entropy Scaled (min)",
                                torch.min(policy_entropy_scaled).item())
                self.track_data("Policy / Entropy Scaled (mean)",
                                torch.mean(policy_entropy_scaled).item())
